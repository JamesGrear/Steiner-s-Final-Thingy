...
public class Transaction implements Initializable
{
    ...
    @FXML private void tryPurchase(ActionEvent event)
    {
		...
		String warning = ""; // warning with warning messages for all items
		...
		try
		{
			...
			String warningMessage; 	// warning message of an individual item
			...
			for(int i = 0; i < 5; i++)
			{
				if(processItem[i])
				{
					...
					warningMessage = item[i].getWarningMessage();

					if(warningMessage.isEmpty() == false)
					{
						warning = warning.concat(String.format("%s may be dangerous to %s:\n%s\n\n", item[i].getName(), customer.getName(), warningMessage));
					}
		...
		{
			Alert drugWarning = new Alert(Alert.AlertType.WARNING);
			drugWarning.initStyle(StageStyle.UTILITY);
			drugWarning.setTitle(null);
			drugWarning.setHeaderText("WARNING: Please alert customer of the following unfavorable drug interactions/side effects.");
			drugWarning.setContentText(warning);

			drugWarning.showAndWait();
		}
